...
const LinkGroup = ({ children, header }) => {
  return (
    <div className="w-full px-4 sm:w-1/2 md:w-1/2 lg:w-3/12 xl:w-2/12 [&:nth-child(3)]:lg:w-4/12 [&:nth-child(3)]:xl:w-3/12">
      <div className="mb-16">
        <h3 className="mb-9 text-xl font-bold uppercase text-dark dark:text-white">
          {header}
        </h3>
...
const NavTag = ({ link, name }) => {
  return (
    <li className="mb-3 mr-3">
      <a
        href={link}
        className="inline-block rounded bg-primary/[0.08] px-[14px] py-[5px] text-base text-dark transition-all hover:bg-primary hover:text-white dark:bg-dark-2 dark:text-white"
      >
        {name}
      </a>
    </li>
...
const ProductsItem = ({ img, link, title, subtitle, price }) => {
  return (
    <div className="flex">
      <div className="mr-5 h-[60px] w-full max-w-[60px] overflow-hidden rounded">
        <img
          src={img}
          alt="blog"
